...
def train( epochs, train_loader, device, model, criterion, optimizer,tensorboard_path,save_path=C.model_save_path):
    model = model.to(device)
    print("training on ", device)
    for epoch in range(epochs):
        model.train()
        top1 = AvgrageMeter()   #用于存储记录已经预测的目标的准确率的类
        ...
        for i, data in enumerate(train_loader, 0):  # 0是下标起始位置默认为0
            ...
            n = inputs.size(0)      #表示batch_size
            top1.update(prec1.item(), n)    #表示预测正确的概率
            ...
            # writer.flush()
        #模型每训练1个epoch记录一次模型
        torch.save(model.state_dict(), save_path+'MyNet epoch:{0}.pth'.format(epoch+1))
